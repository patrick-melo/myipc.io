...
function psql_create_config(database_url)
{
    const options = url.parse(database_url, { parseQueryString: true }).query;

    /* remove the query string */
    if (database_url.indexOf("?") > 0) {
        database_url = database_url.substring(0, database_url.indexOf("?"));
    }

    /* put the cleaned url in the config */
    const config = {
        connectionString: database_url,
    };

    /* add the ssl config if necessary (Aiven) */
    if (options['sslmode'] == 'require') {
        config.ssl = { 
            rejectUnauthorized: true,
            ca: fs.readFileSync("./postgresql/aiven.crt").toString(),
        }
    }

    /* add the ssl config if necessary (Cockroach) */
    if (options['sslmode'] == 'verify-full') {
        config.ssl = { 
            rejectUnauthorized: true,
            ca: fs.readFileSync("./postgresql/cockroach.crt").toString(),
        }
    }
...
function psql_create_session(database_url)
{
    let session = new t_psql_session();
   
    const config = psql_create_config(database_url);

   session.pool = new Pool(config);
...
const PSQLLib = {
    ...
    psql_create_config: psql_create_config,
