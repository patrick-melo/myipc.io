...
async function _file_exists_gif(ipc)
{
    let filename = ipc.token_id;

    filename = await fs.access(IPCGIF_DIR + filename + ".gif")
        .then(res => filename).catch(err => "");

    if (filename != "")
    {
        await fs.unlink(IPCGIF_DIR + filename + ".gif");
        return true;
    }
...
async function ipcgif_store(ipc)
{
    ...
    console.log("Check sprite : " +ipc.token_id);
    _file_exists(ipc);

    let fileExist = _file_exists_gif(ipc);
    if(fileExist)
    {
        console.log("file exists: " +ipc.token_id);
        return filename;
    }
    
    console.log("file not exists: " +ipc.token_id);
    ...
    if (result == -1) return "";
        ...
        console.log("Sprite generated : " +ipc.token_id);
    }
    return filename;
}
